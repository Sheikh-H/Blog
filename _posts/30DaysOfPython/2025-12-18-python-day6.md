...
So far, we've explored what a *collection data type* is and how we can use them with a traditional list. Now, I need to be able to understand how **tuples** differ. Before I begin, I should make mention that a tuple is ordered[^f1] and immutable; once you create a tuple you cannot add, insert, remove, or edit data.
...
To answer that, I often tell myself to relate it to indexing values. If all the items have an index number that starts from 0, then surely the length of the list would match the index + 1. So if I were to create a function that calulcates the entire list and then takes 1 away from the total, I can now use that to reference the last element on the list.

This is especially useful when dealing with tuples beecause mostly you'll be using functions to access different items in the list.
